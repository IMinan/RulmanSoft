...
<?php if(isset($_POST['update_corporate_account'])): ?>
	...
	form_string_control($corporate['gsm'], 'Cep telefonu', array('required'=>true, 'is_gsm'=>true));
		...
		if(is_user("gsm='".$corporate['gsm']."' AND id NOT IN ('".active_user('id')."')") > 0) {
			array_push($err_msg, '<strong>'.$corporate['gsm'].'</strong> telefon numarasına ait üyelik bulunmakta. Üyelik ile ilgili problem yaşıyorsanız <a href="'.account_url('iletisim').'" target="_blank">iletişim</a> sayfasından bizimle iletişime geçebilirsiniz.');
		}
		elseif(is_user("email='".$corporate['email']."'  AND id NOT IN ('".active_user('id')."") > 0) {
			array_push($err_msg, '<strong>'.$corporate['email'].'</strong> e-posta numarasına ait üyelik bulunmakta. Üyelik ile ilgili problem yaşıyorsanız <a href="'.account_url('iletisim').'" target="_blank">iletişim</a> sayfasından bizimle iletişime geçebilirsiniz.');
		}
		else
		{
			if(!is_form_error())
	...
	<div class="row">
		...
		<div class="col-md-6">
			<div class="bg-warning padding10 fs-12">
				Bu panelden sadece profil bilgilerinizi güncelleyebilirsin. Şifreni değiştirmek için <a href="<?php echo account_url('edit_password.php'); ?>">Şifre Değiştir</a> sayfasını ziyaret et.
			</div>
		</div> <!-- /.col-md-6 -->
	</div> <!-- /.row -->
</form>
